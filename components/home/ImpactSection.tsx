          ...
          {impactMetrics.map((metric, index) => {
            ...
            return (
              <AnimatedSection key={index} delay={0.1 * index}>
                <div className="bg-[#8EBA3E] rounded-2xl p-6 hover:shadow-2xl transition-all duration-300 border-2 border-[#8EBA3E]/50">
                  <div className="w-12 h-12 rounded-full bg-white/20 flex items-center justify-center mb-4">
                    <Icon className="h-6 w-6 text-white" />
                  </div>
                  <div className="text-3xl font-bold text-white mb-2">{metric.value}</div>
                  <div className="text-lg font-semibold text-white mb-2">{metric.title}</div>
                  <div className="text-sm text-white/90">{metric.description}</div>
                </div>
              </AnimatedSection>
            );
          })}
